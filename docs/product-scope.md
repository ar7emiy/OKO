...
- **Client-side keyless-party resolution (the central fork) — RESOLVED.** Entity resolution of client *actors* against the reference graph is **core product, not optional and not consultancy** — it is the mechanism that lets a client's messy mention (e.g. an attorney name string) latch onto scraped fraud signal. The clarifying distinction (see [`client-data-standard.md`](./client-data-standard.md) §2): **Category-A public actors** (providers, attorneys, firms, facilities) are *resolved* against the reference graph; **Category-B private individuals** (claimants) are *pseudonymized* by the client and only linked internally — disjoint sets, no contradiction. The client's job is to **surface** actors as structured rows (provider NPIs, an attorney name in a parties table); OKO **resolves** them, automatically, in-environment. "Not consultancy" survives because resolution is an automated pipeline step running on the client's infra, and we still never clean their claim fields. Note-NER (Tier 3) is the fallback for surfacing actors that exist only in free text. Residual guardrail: precision-first thresholds + confidence-carried edges + review band, because false merges manufacture fake rings.
