...
- **Client-side keyless-party resolution (the central fork) — RESOLVED.** Entity resolution of client *actors* against the reference graph is **core product, not optional and not consultancy** — it is the mechanism that lets a client's messy mention (e.g. an attorney name string) latch onto scraped fraud signal. The clarifying distinction (see [`client-data-standard.md`](./client-data-standard.md) §2): **Category-A public actors** (providers, attorneys, firms, facilities) are *resolved* against the reference graph; **Category-B private individuals** (claimants) are *pseudonymized* by the client and only linked internally — disjoint sets, no contradiction. The client's job is to **extract and surface** actors/events as structured rows — *including building their own NER over notes/documents* to surface legal entities and behavioral events; OKO **resolves** them against the reference graph and scores, automatically, in-environment. So **client-side note-NER is the client's extraction responsibility** (a tier-maturity ask), not an OKO capability — which retires the earlier "do we run NER on client notes" fork. "Not consultancy" survives because resolution/modeling is automated and we never clean their claim fields. We additionally seek per-entity behavioral history (`entity_events`) and unstructured narrative embeddings (natively supported via `data[ntype].note_emb` on every node type), under temporal-leakage and Category-A/B privacy rails. Residual guardrail: precision-first thresholds + confidence-carried edges + review band, because false merges manufacture fake rings. See [`client-data-standard.md`](./client-data-standard.md).
